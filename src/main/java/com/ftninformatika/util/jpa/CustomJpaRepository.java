package com.ftninformatika.util.jpa;
...
public interface CustomJpaRepository<T, ID extends Serializable> extends CustomCrudRepository<T, ID> {
  ...
  /**
   * Deletes the given entities in a batch which means it will create a single {@link javax.persistence.Query}. Assume
   * that we will clear the {@link javax.persistence.EntityManager} after the call.
   *
   * @param entities
   */
